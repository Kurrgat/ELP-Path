...
public class SportLightServiceImpl implements SportLightService {
    ...
    public CustomResponse<List<SportLight>> getAll() {
        ...
        try {
            var result = sportLightRepository.findAll();

            List<SportLight> sportLightsWithImages = result
                    .stream()
                    .filter(sportLight -> sportLight.getImage() != null && !sportLight.getImage().isEmpty())
                    .collect(Collectors.toList());

            if (sportLightsWithImages.isEmpty()) {
                response.setStatusCode(HttpStatus.NOT_FOUND.value());
                response.setMessage("No spotlight with images found");
                response.setPayload(null);
                response.setSuccess(false);
            } else {
                for (SportLight sportLight : sportLightsWithImages) {
                    String imageUrl = getImagesPath() + sportLight.getImage();
                    sportLight.setImage(imageUrl);
                }
                response.setPayload(sportLightsWithImages);
