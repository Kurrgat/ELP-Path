...
//                .scholarId(user.getScholar().getId())
    ...
    public CustomResponse<?> verifyScholarEmail(ScholarAuthDto scholarAuthDto) {
...
//                CustomResponse<?> scholarAuthResp = joinProcedure.authenticateUserInput(scholarAuthDto.getScholarPfNo());
//                Map<?, ?> payload = (Map<?, ?>) scholarAuthResp.getPayload();
                ...
                if (usersOptional.get().getUserEmail() != null && !usersOptional.get().getUserEmail().isEmpty()) {
                    ...
                    if(!userEmail.equals(scholarAuthDto.getScholarEmail())) {
                        ...
                        var result = authenticate(authRequest);
...
//                roleRepository.save(role);
        ...
        if (whatWeHave.equals("an unregistered scholar")) {
           ...
           // newUser = scholar.getUser();
            ...
            Optional<Scholar> possibleElp = scholarRepo.findScholarByPfNumber(registerRequest.getData().getScholarId());
            Elp elp = optionalElp.get();
            elp.setUser(user);
            elpRepository.save(elp);
            newUser = elp.getUser();
        }*/
