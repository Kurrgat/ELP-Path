...
public class LikeServiceImpl implements  LikeService{
    ...
    @Override
    public CustomResponse<?> deleteLike(Long likeId) {
        CustomResponse<LikeEntity> response = new CustomResponse<>();
        try {
            // Check if the like exists
            LikeEntity existingLike = likeRepository.findById(likeId).orElse(null);

            if (existingLike == null) {
                response.setMessage("Like with id " + likeId + " not found");
                response.setStatusCode(HttpStatus.NOT_FOUND.value());
                response.setSuccess(false);
                return response;
            }
    ...
    public CustomResponse<?> getAllLikes(Long feedId) {
        CustomResponse<List<LikeEntity>> response = new CustomResponse<>();
        try {
            if (feedId == null) {
                throw new IllegalArgumentException("Feed ID must be provided");
            }

            List<LikeEntity> likes = likeRepository.findCommentsByFeedId(feedId);
