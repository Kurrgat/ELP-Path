...
public class ChapterController {
    ...
    @PutMapping("/v2/set-leader/{chapterId}/{scholarId}")
    ResponseEntity<?> setChapterLeader(@PathVariable Long chapterId, @PathVariable Long scholarId) {
        var result = chapterService.setChapterLeader(chapterId, scholarId, false);
        return ResponseEntity.status(result.getStatusCode()).body(result);
    }
