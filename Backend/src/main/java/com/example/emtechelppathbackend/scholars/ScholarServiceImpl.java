...
public class ScholarServiceImpl implements ScholarService {
    ...
    @Override
    public CustomResponse<?> getAllScholarsPerYear(String year) {
        CustomResponse<List<Scholar>> response = new CustomResponse<>();
        try {
            List<Scholar> allList = scholarRepository.getAllScholarsPerYear(year);
    ...
    @Override
    public CustomResponse<?> getRegionalScholars(String year) {
        CustomResponse<List<Scholar>> response = new CustomResponse<>();

        try {
            List<Scholar> regionalList = scholarRepository.getRegionalScholars(year);
...
//    }
    ...
    @Override
    public CustomResponse<?> getGlobalScholars(String year) {
        CustomResponse<List<Scholar>> response = new CustomResponse<>();
        try {
            List<Scholar> globalList = scholarRepository.getAllGlobalScholars(year);
    ...
    @Override
    public CustomResponse<?> getLocalScholars(String  year) {
        CustomResponse<List<Scholar>> response = new CustomResponse<>();
        try {
            List<Scholar> localList = scholarRepository.getAllLocalScholars(year);
    ...
    @Override
    public CustomResponse<?> getTvetScholars(String year) {
        CustomResponse<List<Scholar>> response = new CustomResponse<>();
        try {
            List<Scholar> tvetList = scholarRepository.getAllTvetScholars(year);
    ...
    public byte[] generateReportPerYear(String year) throws JRException, IOException {
        List<ScholarReportInterface> allList = scholarRepository.getScholarsPerYear(year);

        try {
            List<ScholarDto1> scholarList = new LinkedList<>();

            for (ScholarReportInterface scholar : allList) {
                ScholarDto1 scholarObj = new ScholarDto1();
                scholarObj.setName(scholar.getName());
                scholarObj.setBranch(scholar.getBranch());
                scholarObj.setCode(scholar.getCode());
                scholarObj.setGender(scholar.getGender());
                scholarList.add(scholarObj);
            }

            // Get InputStream for the JRXML file
            InputStream inputStream = getClass().getClassLoader().getResourceAsStream("scholars.jrxml");

            // Compile report
            JasperReport jasperReport = JasperCompileManager.compileReport(inputStream);

            // Get datasource
            JRBeanCollectionDataSource dataSource = new JRBeanCollectionDataSource(allList);
...
//    @Override
//    public byte[] generateAnnualLocalReport(String year) {
//        List<ScholarReportInterface> allList = scholarRepository.getLocalScholarsPerYear(year);
//
//
//        try {
//
...
//
//            //get file and compile it
//            File file = ResourceUtils.getFile("classpath:scholars.jrxml");
//            JasperReport jasperReport = JasperCompileManager.compileReport(file.getAbsolutePath());
//
//            //get datasource
//            JRBeanCollectionDataSource dataSource = new JRBeanCollectionDataSource(allList);
