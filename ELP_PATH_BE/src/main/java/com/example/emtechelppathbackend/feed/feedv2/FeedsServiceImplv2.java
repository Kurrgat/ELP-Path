...
import com.example.emtechelppathbackend.comment.CommentRepository;
...
class FeedsServiceImplv2 implements FeedsServicev2 {
    ...
    @Override
    public CustomResponse<?> getFeedDetailsById(Long feedId) {
        CustomResponse<List<FeedsRepositoryv2.FeedsInterface>> response=new CustomResponse<>();
        try {
            List<FeedsRepositoryv2.FeedsInterface>feeds=feedsRepository.findFeedDetailsById(feedId);
            if (!feeds.isEmpty()){

                response.setMessage("feed found with comments and likes");
                response.setSuccess(true);
                response.setStatusCode(HttpStatus.OK.value());
                response.setPayload(feeds);
            }else {
    ...
    @Override
    public CustomResponse<?> getAllFeeds() {
        CustomResponse<List<FeedsRepositoryv2.FeedsInterface>> response=new CustomResponse<>();
        try {
            List<FeedsRepositoryv2.FeedsInterface>feeds=feedsRepository.getAllFeeds();
            if (!feeds.isEmpty()){
        ...
        try {
            ...
            if (userOptional.isPresent()) {
                ...
                if (feedOptional.isPresent()) {
                    // Delete related like_entity records
                    likeRepository.deleteByFeedId(id);
            ...
            response.setMessage("An error occurred while deleting the feed"+e.getMessage());
            response.setStatusCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
            response.setSuccess(false);
        }
        return response;
