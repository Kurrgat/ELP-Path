...
public class CommentServiceImpl implements CommentService{
    ...
    public CustomResponse<?> getAllComments(Long feedId) {
        CustomResponse<List<CommentRepository.CommentInterface>> response = new CustomResponse<>();
        try {
            if (feedId == null) {
                throw new IllegalArgumentException("Feed ID must be provided");
            }

            List<CommentRepository.CommentInterface> comments = commentRepository.findCommentsByFeedId(feedId);

            if (comments == null || comments.isEmpty()) {
                // No comments found
                response.setMessage("No comments found for the specified feed");
                response.setStatusCode(HttpStatus.NOT_FOUND.value());
                response.setSuccess(false);
                response.setPayload(null);
            } else {
            ...
            // Error occurred while retrieving comments
