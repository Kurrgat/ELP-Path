...
public class ProfileProgressService {
    ...
    public CustomResponse<ProgressResponse> getProfileUpdateStatus(Long userId) {
        ...
        try {
            ...
            if(optional.isEmpty()) {
                ...
                int achievementsPerc=calculateCareerCompletionPercentage(userId).getPayload().percentageDone;

                progressPercentage = (profilePerc+bioPerc+educationPerc+skillsPerc+careerPerc+achievementsPerc)/5;
                ...
                profileProgress.setAchievementsTracker(calculateAchievementsCompletionPercentage(userId).getPayload());
    ...
    public CustomResponse<ProfileTracker> calculateProfileCompletionPercentage(Long userId) {
        ...
        int totalAttributes = 10; // Total number of attributes in the class
    ...
    public CustomResponse<EducationTracker> calculateEducationCompletionPercentage(Long userId) {
        ...
        ArrayList<String> pending = new ArrayList<>();
        int totalAttributes = 4; // Total number of attributes in the class
...
public CustomResponse<AchievementsTracker>calculateAchievementsCompletionPercentage(Long userId){
        CustomResponse<AchievementsTracker>response=new CustomResponse<>();
        AchievementsTracker progressResponse=new AchievementsTracker();
        ArrayList<String> pending = new ArrayList<>();
    int totalAttributes = 1;// Total number of attributes in the class
    int addedValues = 0;
    try {
        List<Achievements> achievements = achievementsRepository.findByUserId(userId);
        ...
        if(achievements != null) {
            if (!achievements.isEmpty()) {
                ...
                pending.add("Kindly update your achievements details");
            }
        } else {
            pending.add("User hasn't added any achievements details yet");
            progressResponse.setPercentageDone(0);

            response.setMessage("Add your achievements details");
            response.setStatusCode(HttpStatus.NOT_FOUND.value());
            response.setPayload(progressResponse);
            return response;
        }
        ...
        response.setMessage("error calculating percentage" +e.getMessage());
        response.setStatusCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
        response.setSuccess(false);
    }
    return response;
