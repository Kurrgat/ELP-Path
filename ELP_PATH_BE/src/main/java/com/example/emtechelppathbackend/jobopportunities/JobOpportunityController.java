...
public class JobOpportunityController {
    ...
    @PostMapping("/{userId}/{jobId}/easyApply")
    public ResponseEntity<?> easyApply(@PathVariable Long userId, @PathVariable Long jobId) {
            var response = jobOpportunityService.easyApply(userId,jobId);
            return ResponseEntity.status(response.getStatusCode()).body(response);

    }

    @GetMapping("/count-job-applications/{opportunityId}")
    public ResponseEntity<?> countJobApplications(@PathVariable Long opportunityId) {
        var response = jobOpportunityService.countJobApplications(opportunityId);

        return ResponseEntity.status(response.getStatusCode()).body(response);
    }

    @GetMapping("search")
    public ResponseEntity<?> searchOpportunity(@RequestParam("q") String keyword) {
        var response = jobOpportunityService.searchOpportunity(keyword);
        return ResponseEntity.status(response.getStatusCode()).body(response);
    }
}
