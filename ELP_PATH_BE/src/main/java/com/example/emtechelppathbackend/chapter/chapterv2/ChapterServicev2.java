...
public interface ChapterServicev2 {
    ...
    CustomResponse<?> joinChapter(Long userId);
    CustomResponse<?> updateChapterByChapterId(Long chapterId, ChapterDTOUpdatev2 updatedChapterDto, MultipartFile chapterImage) throws NoResourceFoundException, IOException;
    CustomResponse<?> leaveChapter(Long userId, Long chapterId);
    CustomResponse<?> deleteChapter(Long chapterId);
    CustomResponse<List<UsersDto>> getMembersByChapterId(Long chapterId) throws NoResourceFoundException;

    CustomResponse<?> fetchChapterHandlers();
    CustomResponse<?> fetchChaptersThatScholarBelongsTo(Long userId);

    CustomResponse<?> setChapterLeader(Long chapterId, Long scholarId, boolean b);

    CustomResponse<?> getChaptersWithMembers();
}
